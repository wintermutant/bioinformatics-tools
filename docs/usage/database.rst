...
super niave changelog. Think of it as a receipt (or transaction history) with a very brief account of all the activity
...
.. list-table::
   ...
   * - Type
     ...
     - Timestamp
   * - db_name
     - top-level name for the database
     - Example database
     - 15Jan2020
   * - creation_date
     - timestamp db was first created
     - 01Jan2000:08:20:20
     - 01Jan2000
   * - activity
     - example_checksum-hd5120: performed summary stat
     - dane basic stats type: fasta db: example.db
     - 19Jan2020
   * - log_entry
     - log text entry
     - a bunch of sample log text...
     - 19Jan2020
   * - activity
     - example_checksum-hd5120: performed annotation
     - dane annotate type: fasta db: example.db annotations: tigerfam
     - 20Jan2020
   * - checksum_contents
     - checksum for file contents
     - example_checksum-hd5120
     - 15Jan2020
   * - checksum_contents
     - checksum for file contents
     - example_checksum-hd5899
     - 01Jan2020
...
contents in the database have not changes so we can expect the same result. To verify, we check that the checksum
...
Note that our database functionality is not meant to contain extensive version controlling beyond what sqlit3 already
