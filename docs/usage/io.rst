...
n: 20, we can outut the 20 largest seqs to the output: out.fasta file.
...
Above, we can just specify a top-level yaml mapping called *n_largest_seqs* and then underneith have nested values to
...
**TODO (LOGIC):** In future updates, you will be able to specify both global profiles (configs) as well as project-specify
...
Following my spiel on logging and reproducibility, we further extend our principles into reducing redudant computations. For sake
...
database file we would do no computation and instead return the stored result - with one cavaet. When we first ran *n largest seqs*
...
see the :doc:`Database Section <database>` 

.. .. code-block:: python

..    from bioinformatics_tools.file_classes import FastqFile

..    # Read a FASTQ file
..    fastq = FastqFile("input.fastq")
..    for record in fastq:
